...
nmf_output_folder <- "/projects/0/einf2548/cruiz/dmg/notebooks/nmf/nmf_myeloid" # Folder to store the results.

if (!file.exists(nmf_output_folder)) dir.create(nmf_output_folder)

sample.myeloid <- readRDS('/projects/0/einf2548/cruiz/dmg/notebooks/nmf/data/myeloid_for_nmf.rds')
sample.myeloid

# Subset the myeloid population pertaining to the sample.
myeloid <- sample.myeloid[, sample.myeloid$sample == sample_index]
myeloid <- myeloid %>% FindVariableFeatures(nfeatures = 7000) 
myeloid <- subset(myeloid, features = VariableFeatures(myeloid))

id <- unique(myeloid$SampleID)

message("Processing sample ", id)

# Re-center the genes for this sample.
myeloid <- Seurat::ScaleData(myeloid, features = rownames(myeloid))

# Extract the count matrix.
cts <- Seurat::GetAssayData(myeloid, slot = 'scale.data')
